...
describe("4 - Implemente os casos de teste para a função `circle`", () => {
  it("Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados", () => {
    expect(circle("8")).toBe(undefined);
  });
  ...
  it("Testa se quando a função não recebe nenhum parâmetro, retorna undefined", () => {
    expect(circle()).toBe(undefined);
  });

  it("Testa se a função retorna uma `key` com `value` igual à circunferência correta para um círculo de raio 2", () => {
    expect(circle(2).circumference).toBeCloseTo(12.56);
  });

  it("Testa se a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3", () => {
